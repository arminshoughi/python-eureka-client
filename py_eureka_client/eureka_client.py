...
class EurekaClient:
    ...
    * strict_service_error_policy: When set to True, all error(Including connection error and HttpError, like http
    ...
    async def __parepare_instance_info(self):
        ...
        mdata = {"management.port": str(self.__instance_port)}
        if self.__eureka_server_conf.zone:
            mdata["zone"] = self.__eureka_server_conf.zone
        mdata.update(self.__metadata)
        ins_id = (
            self.__instance_id
            or f"{self.__instance_ip}:{self.__app_name.lower()}:{self.__instance_port}"
        )
        ...
        self.__instance = {
            ...
            "homePageUrl": EurekaClient.__format_url(
                self.__home_page_url, self.__instance_host, self.__instance_port
            ),
            "statusPageUrl": EurekaClient.__format_url(
                self.__status_page_url,
                self.__instance_host,
                self.__instance_port,
                "info",
            ),
            "healthCheckUrl": EurekaClient.__format_url(
                self.__health_check_url,
                self.__instance_host,
                self.__instance_port,
                "health",
            ),
            ...
            "isCoordinatingDiscoveryServer": str(
                self.__is_coordinating_discovery_server
            ).lower(),
        }
        if self.__data_center_name == "Amazon":
    ...
    async def cancel(self) -> None:
        try:
            ...
            async def do_cancel(url):
                await cancel(url, self.__instance["app"], self.__instance["instanceId"])
